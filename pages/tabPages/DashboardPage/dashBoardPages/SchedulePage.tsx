import { Box, FlatList, HStack, Heading, VStack, Text, Accordion, ScrollView, Input, Spinner, Flex } from "native-base";
...
import { SortedEvents, sortEventsByDate } from "../../../../Services/Helpers/sortEventsBydate";
...
export function SchedulePage({ type }: { type: reqType }) {
	...
	const [selectedDay, setSelectedDay] = useState<string>(getDayOfWeek(new Date().toLocaleDateString()));
	...
	return (
		<VStack>
			...
			<ScrollView horizontal={false} flex={1}>
				<Flex direction='row' justifyContent={"space-around"} marginTop={3} marginBottom={1}>
					<AntDesign
						name='leftcircleo'
						size={24}
						color='black'
						onPress={() =>
							setCurrentWeekDay(new Date(currentWeekDay.setDate(currentWeekDay.getDate() - 7)))
						}
					/>
					...
					<Flex>
						{schedule[selectedDay].map((item) => (
							<View
								...
								<Text numberOfLines={1} color={"white"} fontSize={20}>
									{item.nameEt}
								</Text>
								<Flex direction='row' justifyContent={"space-between"}>
									<Text numberOfLines={1} color={"white"}>
									...
									<Text numberOfLines={1} color={"white"}>
										{item.teachers.map((teacher) => (teacher.name ? teacher.name : "-")).join(" ")}
									</Text>
								</Flex>
							</View>
						))}
...
export function GroupListSelection({
	...
	return (
		<Box>
			...
			<ScrollView marginTop={'3px'}>
				{enableSuggestions &&
					suggestions.map((item) => (
						<TouchableOpacity key={item.id} onPress={() => handleSuggestionClick(item)}>
							<Box backgroundColor={"gray.100"} marginY={"1px"} padding={"10px"} borderRadius={'10px'}>
								<Text>
									{"nameEt" in item ? item.nameEt : "name" in item ? item.name : item.id.toString()}
								</Text>
							</Box>
						</TouchableOpacity>
					))}
